...
def save_binary_chunks(file_path, header, data, output_dir="./binary_chunks"):
    ...
    for lon_offset in range(0, ncols, step * factor):
        for lat_offset in range(0, nrows, step * factor):
            ...
            lat = yllcorner + nrows * header["cellsize"] - lat_offset * header["cellsize"]
...
async def query_population(request):
    ...
    for chunk_index, chunk_file in enumerate(chunk_files):
        ...
        lat_max = float(parts[1])  # 注意：这里是块的上边界
        lon_max = lon_min + 10
        lat_min = lat_max - 10  # 注意：这里是块的下边界
                ...
                for i in range(chunk.shape[0]):
                    for j in range(chunk.shape[1]):
                        ...
                        if np.isnan(cell_value):
                            continue
                        x_min = lon_min + j * (1 / 120)
                        y_max = lat_max - i * (1 / 120)
                        cells.append({
                            "longitude": round(float(x_min), 5),
                            "latitude": round(float(y_max), 5),
        ...
        for i in range(chunk.shape[0]):
            for j in range(chunk.shape[1]):
                ...
                y_max = lat_max - i * (1 / factor)
                cell_polygon = Polygon([
                    (x_min, y_max - (1 / factor)),
                    (x_min + (1 / factor), y_max - (1 / factor)),
                    (x_min + (1 / factor), y_max),
                    (x_min, y_max),
                ])
